...
Either open **`obx.html`** — one self-contained file, no server, no
dependencies — or serve the modular source:
...
`obx.html` is generated by `npm run build` and is the only form that runs
straight from disk: ES modules and worklets cannot be fetched over `file://`,
so the modular `index.html` needs the server. The bundle carries its processor
as inert text and turns it into a URL at start-up, which sidesteps that. It
performs identically to the modular build — measured, not assumed.
...
| `build.mjs` | Bundles everything into the standalone `obx.html` |
...
Five suites run against real Chromium: a layout and render check over all 32
factory programs, spectral measurements of the synthesis engine, alias-to-signal
measurements on saw, pulse and sync patches, DOM-driven behaviour checks on the
programmer and keyboard routing, and a bundle check that rebuilds `obx.html`
and confirms it boots, sounds and performs the same over `http://` and
`file://`. 68 assertions in total.
