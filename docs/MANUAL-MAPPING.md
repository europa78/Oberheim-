...
| --- | --- | --- | --- |
| VOLUME | `g:masterVol` (`GLOBALS.masterVol`, default 0.75) | 0..1, knob, not programmable | *"This control affects the output volume of the OB-X simultaneously from the LEFT, MONO, and RIGHT outputs … this parameter cannot be input into the programmer."* |
...
| — (addition) | `g:volBalance` (`GLOBALS.volBalance`, default 0.5) | 0..1, described in `params.js` as "stereo spread of the voice pan-pots" | Not a 1979 panel control; from the plugin skin. |
| MASTER TUNE | `g:masterTune` (`GLOBALS.masterTune`, default 0.5) | 0..1 with a dead zone at centre = A-440; the `cents` formatter reads out `(v-0.5)*100`, i.e. ±50 cents | `params.js` comment: "dead-zone at centre = A-440". The manual page consulted covers VOLUME/AUTO/HOLD/CHORD only. |
...
| Manual control | Parameter id | Range / behaviour in this build | Manual quote or note |
| --- | --- | --- | --- |
| DEPTH | `depth1` | knob, default 0.00 | *"This control determines the amount of selected LFO waveform to be sent to modulate the frequency of Oscillator 1, Oscillator 2 or the Filter, as controlled by the destination switches below it."* |
...
| Manual control | Parameter id | Range / behaviour in this build | Manual quote or note |
| --- | --- | --- | --- |
| DEPTH | `depth2` | knob, default 0.00 | Amount for the PULSE WIDTH destination column. `d2Pwm1` / `d2Pwm2` do nothing unless `depth2` > 0. |
...
| VOLUME | `d2Volume` | switch, default 0 | Third destination in this column on the panel (LFO to loudness — tremolo). The manual page consulted stops after the FREQUENCY column's OSC 1 entry, so no quote is offered for it. |
...
| PULSE WIDTH | `pulseWidth` | knob, default 0.00; shared by both oscillators; the `duty` formatter reads `50 − 45v` %, i.e. 50 % at 0 and 5 % at 1 | *"This control allows selection of initial pulse width of both oscillators. When it is set fully counter-clockwise a square wave (50 % duty cycle) is selected. When it is set fully clockwise a 5 % duty cycle is selected."* Has no effect unless a Pulse waveform is on. |
| X-MOD | `xmod` | knob, default 0.00 | *"When switched on, causes Oscillator 2 to modulate Oscillator 1. This allows for production of 'ring-modulator' type sounds."* **Difference:** the manual describes a switch; this build exposes it as a continuous amount knob (with SYNC as the switch beside it). |
...
| Manual control | Parameter id | Range / behaviour in this build | Manual quote or note |
| --- | --- | --- | --- |
| FREQUENCY | `cutoff` | knob, default **1.00** (wide open) | *"This control determines the initial cut-off frequency of the Filter. The Filter is a two-pole, low-pass type."* |
| RESONANCE | `resonance` | knob, default 0.00 | *"This control determines the amount of resonance ('Q' or 'emphasis') of the Filter. Note that even in its maximum position, the Filter cannot be put into oscillation."* — i.e. no self-oscillation at maximum, unlike a Moog-style ladder. |
| MODULATION | `filterMod` | knob, default 0.00 | *"This control determines the amount of Filter Envelope which modulates the Filter. The control works independently from and in addition to the amount of LFO modulation from the MODULATION section."* |
...
| OSC 2 HALF / FULL | `fOsc2` | multi, 3 states, default **2** — 0 = off, 1 = half, 2 = full | *"The FULL switch selects the full output of the Oscillator and the HALF switch selects a signal level approximately 5 db. below full output."* Two panel switches on the original; one three-state control with `HALF`/`FULL` LED caps here. |
| NOISE HALF / FULL | `fNoise` | multi, 3 states, default 0 — off / half / full | *"The Full switch selects the full output of the Noise Generator and the HALF switch selects a signal level approximately 5 db. below full output."* |
| KBD | `kbdTrack` | switch, default 1 | *"This switch determines whether or not the keyboard control voltage going to each voice … is applied to the frequency control input of the Filter. When on, causes the Filter in each voice to 'track' the keyboard."* |
...
All eight knobs are plain 0..1. The `msTime` formatter used for the six time
knobs reads `1 + 9999·v³` ms — about 1 ms fully counter-clockwise to 10 s fully
clockwise — which matches the manual's *"The shortest time is selected by
setting it fully counter-clockwise."*
...
| NARROW / BROAD | `g:bendRange` (`GLOBALS.bendRange`, default 0) | 0 = narrow, 1 = broad; panel caption is AMOUNT | *"In the NARROW position, the PITCH BEND lever has a range of up or down one whole-step … In the BROAD position, the PITCH BEND Lever can move the pitch up or down one octave."* |
